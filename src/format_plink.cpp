...
bool format_plink::write_ped01_map(std::string file_out_name, std::vector<std::vector<bool> > &matrix_plink_ped, plink_PED_ids &plink_ped_ids, plink_MAP &plink_map)
{
    // create .ped and .map files
    // create a file for each chromosome
    std::string sep=" ";
    ...
    for (unsigned long int i=0; i<nind; i++)
    {
        ...
        {
            // true (1) means al1, false (0) means al0
            std::string s1=matrix_plink_ped[i][2*j] ? "1" : "0";
            std::string s2=matrix_plink_ped[i][2*j+1] ? "1" : "0";
            file_ped << sep << s1 << sep << s2;
        }
        file_ped << std::endl;
    }
